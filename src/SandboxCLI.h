...
inline void PrintUsage(const char* version)
{
    printf(
        ...
        "  All 7 keys required. Use [] for no grants. Absolute paths only, must exist on disk.\n"
        "  Grants (except peek) are recursive: a directory grant applies to ALL descendants.\n"
        "  Peek grants access ONLY to the named directory — no inheritance to children.\n"
        "\n"
        "  [deny]                                 (required, both modes)\n"
        "  Same 6 keys as [allow] (no peek). Block access within a broader grant.\n"
        "  Deny is recursive: applies to the path and all descendants.\n"
        "  Allow-inside-deny: an [allow] under a [deny] carves out an allowed subtree.\n"
        ...
        "  Best-effort: parent registry key cascade (Software\\Sandy\\Grants, Software\\Sandy),\n"
        "    stale scheduled task cleanup, desktop/WinSta ACL cleanup (may fail in\n"
