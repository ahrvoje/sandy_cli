...
inline void PrintUsage(const char* version)
{
    printf(
        ...
        "\n"
        "  [sandbox]                              (required)\n"
        "  token = 'appcontainer'                 # required: 'appcontainer', 'lpac', or 'restricted'\n"
        "  # integrity = 'low'                    # required (restricted only): 'low' or 'medium'\n"
        "  # strict = false                       # optional (restricted only): exclude user SID from restricting list\n"
        "  # workdir = 'inherit'                  # optional (or absolute directory path; default: inherit Sandy's current working directory)\n"
        "\n"
        "  [allow.deep]                           (optional, recursive grants)\n"
        "  All access levels: read, write, execute, append, delete, all, run, stat, touch, create\n"
        "  Grants apply recursively to the directory and ALL descendants (OI|CI inheritance).\n"
        "\n"
        "  [allow.this]                           (optional, single-object grants)\n"
        "  Same access levels. Grant applies ONLY to the object itself, not children.\n"
        "  Uses SetKernelObjectSecurity (instant, no child walk).\n"
        "\n"
        "  All keys optional (default: []).  Absolute paths only, must exist on disk.\n"
        "\n"
        "  [deny.deep]                            (restricted token only, recursive deny)\n"
        "  [deny.this]                            (restricted token only, single-object deny)\n"
        "  Same keys as allow. Block access within a broader grant.\n"
        ...
        "\n"
        "  [privileges]                           (optional)\n"
        "  network         = false                # default: false (appcontainer/lpac only)\n"
        "  lan             = false                # false | 'with localhost' | 'without localhost' (appcontainer/lpac only)\n"
        "  named_pipes     = false                # default: false (restricted only)\n"
        "  desktop         = true                 # default: true (restricted only, WinSta0 + Desktop)\n"
        "  stdin           = false                # default: false (NUL). true=inherit, absolute path=file\n"
        ...
        "  --status summarizes instance, retry-container, cleanup-task, and profile state, but is not yet a full requested-vs-effective\n"
...
inline void PrintContainerToml()
{
    printf(
        ...
        "[sandbox]\n"
        "token = 'appcontainer'\n"
        "# workdir = 'inherit'            # default: inherit Sandy's current working directory (or set an absolute directory path)\n"
        "\n"
        "[allow.deep]                      # default: no grants (all keys default to [])\n"
        "# read    = []\n"
        "# write   = []\n"
        "# execute = []\n"
        "# append  = []\n"
        ...
        "# stdin           = false                # true=inherit, absolute path=file\n"
...
inline void PrintRestrictedToml()
{
    printf(
        ...
        "token     = 'restricted'\n"
        "integrity = 'low'\n"
        "# strict    = false                # default: false (user SID in restricting list)\n"
        "# workdir   = 'inherit'           # default: Sandy's current working directory (or set an absolute directory path)\n"
        "\n"
        "[allow.deep]                      # default: no grants (all keys default to [])\n"
        "# read    = []\n"
        "# write   = []\n"
