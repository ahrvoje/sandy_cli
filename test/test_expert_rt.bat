...
REM =====================================================================
REM test_expert_rt.bat -- Expert adversarial test suite (Restricted Token)
REM Targets Sandy's implementation assumptions, not Windows kernel.
REM =====================================================================

echo =====================================================================
echo  Expert Adversarial Test Suite — Restricted Token Mode
...
set "CONFIG=c:\repos\sandy_cli\test\test_expert_rt_config.toml"
set "SCRIPT=c:\repos\sandy_cli\test\test_expert_rt.py"
...
REM --- A1: Check for residual RT SIDs ---
set "SID_FOUND=0"
...
if exist "%ARENA%\runtime_storm" (
    for /f "delims=" %%L in ('icacls "%ARENA%\runtime_storm" /T 2^>nul ^| findstr /i "S-1-9-"') do (
        set "STORM_SID=1"
    )
    if "!STORM_SID!"=="0" (
...
echo =====================================================================
if %SANDY_EXIT% EQU 0 (
    echo  ALL HELD -- Sandy survived expert attacks (RT mode)
) else (
